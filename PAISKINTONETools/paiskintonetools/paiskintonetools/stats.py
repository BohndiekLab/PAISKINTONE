...
class LinearRegDiagnostic:
    ...
    def __init__(
        ...
        if (
            isinstance(results, sm.regression.linear_model.RegressionResultsWrapper)
            is False
        ):
            raise TypeError(
    ...
    def __call__(self, **kwargs):
        ...
        self.leverage_plot(
            ax=ax[1, 1],
            high_leverage_threshold=kwargs.get("high_leverage_threshold"),  # type: ignore
            cooks_threshold=kwargs.get("cooks_threshold"),  # type: ignore
        )
        # plt.show()
    ...
    def residual_plot(self, ax=None):
        ...
        for i in abs_resid_top_3:
            ax.annotate(i, xy=(self.y_predict[i], self.residual[i]), color="C3")  # type: ignore
    ...
    def leverage_plot(
        ...
        ymax = max(abs(min(self.residual_norm)), max(self.residual_norm))
...
def loess_bootstrap(x, y, frac=0.75, n_bootstrap=500, seed=None, it=3, x_eval=None):
    rng = np.random.default_rng(seed=seed)
    if x_eval is None:
        x_eval = np.linspace(np.min(x), np.max(x), 100)

    calculated = np.empty((n_bootstrap, x_eval.shape[0]))

    for i in range(n_bootstrap):
        x_select = rng.choice(len(x), len(x), replace=True)
        x_b = x[x_select]
        y_b = y[x_select]
        Y = sm.nonparametric.lowess(exog=x_b, endog=y_b, frac=frac, xvals=x_eval, it=it)
        calculated[i] = Y
    ...
    return (
        x_eval,
        all_points,
        np.percentile(calculated, 97.5, axis=0),
        np.percentile(calculated, 2.5, axis=0),
    )


def p_formatter(p):
    if p < 0.001:
        return r"$\mathdefault{p < 0.001}$"
    else:
        return rf"$\mathdefault{{p = {p:.3f}}}$"
